...
Phase 2-Eでは、次の実験前準備としてlow-risk detail-dense専用target datasetを追加しました。`npm run agent:routing:contract:target-calibrate`は外部APIを呼ばず、8件すべてが`single_pass`かつ`lightweightChecklistRecommended=true`になることを確認します。
