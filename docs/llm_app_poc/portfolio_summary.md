...
- 保存履歴を利用した評価資料作成
- `LLM_PROVIDER=mock | openai | gemini` によるprovider切り替え
...
同じ「プロフィール画像変更」要件を、同一スキーマ・同一評価基準で `mock-local`、Gemini実LLM出力、OpenAI実LLM出力として比較した。mock-localは **3.4 / 5**、Geminiは **4.4 / 5**、OpenAIは **4.6 / 5** となり、`5MB`、`JPG/PNG`、`即時反映`、`失敗時エラーメッセージ` といった入力固有条件の反映が大きく改善した。

Geminiはセキュリティ、保存先、スケーラビリティなどリスク観点の幅が広く、OpenAIは受け入れ条件、Jiraチケット分解、即時反映の実装順序がより実務タスクに落とし込みやすかった。provider差分を吸収しながら、同一UI・同一スキーマ・同一評価資料でモデル比較できる状態まで確認できた。
...
- provider差分を吸収し、mock/OpenAI/Geminiの出力を同じ型とUIで扱える。
- Gemini structured outputで、JSON Schemaサブセット、`candidates[].content.parts[].text` の抽出、レスポンス診断を実装している。
- OpenAI/Geminiの実LLM出力を同じ入力・同じ評価軸で比較し、モデルごとの得意領域を評価資料に落とし込んでいる。
