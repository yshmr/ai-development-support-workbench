...
レイテンシ評価では、provider latency、server processing、client elapsedを分離して計測した。今回の実測ではserver processingとprovider latencyがほぼ同値で、画面上の待ち時間は主にLLM provider応答待ち時間によることを確認した。品質評価と組み合わせると、今回の特定タスクではClaudeが最高品質、OpenAIが品質と応答時間のバランスに優れる結果になった。
...
- provider latency、server processing、client elapsedを分離し、品質とレイテンシを組み合わせてmodel選定を評価できる。
