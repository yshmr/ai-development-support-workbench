...
## 10. Adaptive Agent Routing Evaluation

Phase 2-Aでは、Phase 1-Eの「Agent workflowはdefault pathではなくselective useが妥当」という結論を受けて、deterministic `agent-routing-v1`を実装・評価した。

Evaluation design:

- Always OFF: single-pass grounded generation
- Always ON: bounded Agent workflow
- Routed: deterministic router chooses `single_pass` or `agent_workflow`
- 24 runs
- 8 runs per mode
- blind manual scoring
- same seven-axis rubric
...
`agent-routing-v1`は今回のdatasetではcost-aware selective orchestrationとして成立しなかった。Routed modeは品質平均でAlways OFF / Always ONを下回り、Agent回避も発生しなかった。token usageもAlways ONより大きかった。

このnegative resultは、routing policy failureとして扱う。deterministic routing schema、routing metadata、evaluation harnessは維持するが、routed modeをdefaultにはしない。

Learning:

- selective Agent useという仮説自体は有効な問題設定だが、今回のdeterministic signalsは保守的すぎた。
- risk / failure / ambiguity marker中心のpolicyは、このdatasetでは全件Agent workflowへcollapseした。
...
## 14. Career Experience Connection
